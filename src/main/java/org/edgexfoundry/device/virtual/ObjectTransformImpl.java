...
public class ObjectTransformImpl implements ObjectTransform {
  ...
  public void setTransformData(boolean transformData) {
	...
	public String transform(PropertyValue value, String result) {
		if ("f".equals(value.getType().toLowerCase()) || "float".equals(value.getType().toLowerCase())) {
			return transformFloat(value, result);
		} else {
			return result;
		}
	}

	private String transformFloat(PropertyValue value, String result) {
		double floatValue;
